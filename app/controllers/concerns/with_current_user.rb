module WithCurrentUser
  ...
  def current_user
    current_jwt_user(nil_on_failure: true) || (session[:current_user_id] && Accounts::User.find(session[:current_user_id]))
  end

  def user_class
    raise "WithCurrentUser.user_class not defined!"
  end

  def user_with(email:, id:)
    Accounts::User.find_or_create_by!(id: id, email: email)
  end
